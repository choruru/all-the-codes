...
fn dfs(from: String, edges: &HashMap<String, Vec<String>>, visited: &mut HashSet<String>) -> i32 {
    if &from == "end" {
        return 1;
    }
    ...
    for to in edges[&from].iter() {
        ...
        if small_cave {
            if visited.contains(to) {
                ...
                count += dfs(to.to_string(), edges, visited);
...
fn main() {
    ...
    let ans = dfs("start".to_string(), &edges, &mut HashSet::new());
    println!("{}", ans);
}
