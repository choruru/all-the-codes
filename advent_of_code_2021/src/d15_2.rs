...
fn main() {
    ...
    // map construction, -1 to make the expansion easier.
    let mut small_map = vec![];
    ...
    let small_ylen = small_map.len();
    let small_xlen = small_map[0].len();

    let ylen = 5 * small_ylen;
    let xlen = 5 * small_xlen;

    // map expansion
    let mut map = vec![vec![0; xlen]; ylen];
    for y in 0..ylen {
        for x in 0..xlen {
            map[y][x] = (small_map[y % small_ylen][x % small_xlen]
                + (y / small_ylen) as i32
                + (x / small_xlen) as i32)
                % 9;
        }
